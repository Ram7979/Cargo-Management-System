...
export class InvoiceCreateComponent implements OnInit {
  ...
  ngOnInit() {
    this.customerService.getAll(1, 200).subscribe(res => {
      if (res.success) this.customers = res.data.items;
    });
    this.shipmentService.getAll(1, 200).subscribe(res => {
      if (res.success) this.shipments = res.data.items;
    });
  }
