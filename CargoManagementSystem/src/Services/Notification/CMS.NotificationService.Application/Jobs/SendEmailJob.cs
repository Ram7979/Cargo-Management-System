...
public class SendEmailJob
{
    ...
    public async Task Execute(Guid notificationId)
    {
        ...
        var success = await _emailService.SendAsync(record.Recipient, record.Subject, record.Body);

        if (success)
        {
            record.MarkSent();
            _logger.LogInformation("Email sent for notification {Id}", notificationId);
        }
        else
        {
            ...
            {
                _logger.LogWarning("Email notification {Id} failed, retry {RetryCount}", notificationId, record.RetryCount);
            }
        }
