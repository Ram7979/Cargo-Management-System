...
export class DriverFormDialogComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.userService.getAll(1, 200).subscribe((res: ApiResponse<PagedResult<any>>) => {
      if (res.success) {
...
export class DriversPageComponent implements OnInit {
  ...
  loadDrivers() {
    ...
    this.driverService.getAll(this.pageIndex + 1, this.pageSize).subscribe({
      next: (res: ApiResponse<PagedResult<Driver>>) => {
        if (res.success && res.data) {
          this.dataSource = res.data.items || [];
          this.totalItems = res.data.totalCount || this.dataSource.length;
