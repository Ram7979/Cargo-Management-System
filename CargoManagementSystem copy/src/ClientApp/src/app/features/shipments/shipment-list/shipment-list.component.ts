...
export class ShipmentListComponent implements OnInit {
  ...
  loadShipments() {
    ...
    const obs$ = this.auth.getUserRole() === 'Customer' 
      ? this.shipmentService.getMyShipments(this.pageIndex + 1, this.pageSize, filters?.trackingNumber)
      : this.shipmentService.getAll(this.pageIndex + 1, this.pageSize, filters);
    ...
    this.shipmentService.cancel(id, 'Cancelled by admin').subscribe({
      next: (res: ApiResponse<any>) => {
        if (res.success) {
          ...
          const msg = res.errors?.join(', ') || res.message || 'Cancellation failed';
