...
@Component({
  ...
  styles: [`
    .w-100 { width: 100%; }
    ::ng-deep .mat-mdc-select-value-text { color: #000000 !important; }
    ::ng-deep .mat-mdc-form-field-outline { color: rgba(0,0,0,0.4) !important; }
    ::ng-deep input.mat-mdc-input-element, ::ng-deep textarea.mat-mdc-input-element { color: #000000 !important; }
  `]
})
export class ShipmentStatusDialogComponent {
  ...
  onConfirm() {
    if (this.statusForm.valid) {
      const result = this.statusForm.value;
      if (result.status === 'Delivered') {
        result.podSignatureData = 'mock_signature_data';
      } else if (result.status === 'Failed Delivery' || result.status === 'FailedDelivery') {
        result.failureReason = 'CustomerUnavailable';
      }
