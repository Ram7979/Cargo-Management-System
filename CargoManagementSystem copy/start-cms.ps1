# CMS Full Stack Startup Script
# Usage: .\start-cms.ps1
# Run from: D:\Files\CMS\CargoManagementSystem

Write-Host "=== CMS Startup Script ===" -ForegroundColor Cyan

# Step 1: Kill all stale service processes
Write-Host "`n[1/3] Cleaning up stale processes..." -ForegroundColor Yellow

Get-Process -Name "dotnet","node" -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue

$allPorts = @(5000,5001,5002,5003,5004,5005,5006,5007,5008,5009,7000,7001,7002,7003,7004,7005,7006,7007,7008,7009,4200)
$stale = Get-NetTCPConnection -LocalPort $allPorts -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess -Unique
...
Write-Host "`n[2/3] Starting backend services..." -ForegroundColor Yellow
...
Start-Process "powershell" -ArgumentList "-NoExit -Command `"cd '$clientPath'; ng serve --configuration=development`"" -WindowStyle Normal
